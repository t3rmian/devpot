...
A basic implementation of the load tests include the following steps:
1. Configuring (optionally parametrizing) connection to the interfaces [*Test Plan/User Variables Config*].
2. Loading input data [*setUp Thread Group*]:
    - from database [*JDBC Config/Sampler*];
    - from CSV file [*CSV Config*].
3. Preparation of test cases that call the interfaces:
...
After sharing the input data, you can randomize it for a single execution and save the necessary information as a variable. It will be later accessible in the same Thread Group using `${variable_name}` syntax.
...
<table class="rwd">
   ...
   <tbody>
      ...
      <tr>
         ...
         <td data-label="Note">
            Similar performance as java.util.Random points that the execution in already thread local in the thread group (no explicit execution in thread pool inside the script)
         </td>
      </tr>
      <tr>
         <td data-label="Randomizer">
         ...
         <td data-label="Throughput [/sec]">
            <span style="color:blue">11704</span>
         </td>
         <td data-label="Note">
      ...
      <tr>
         ...
         <td data-label="Throughput [/sec]">
            <span style="color:red">5065</span>
         </td>
         <td data-label="Note">
            Twice the slowest
         </td>
